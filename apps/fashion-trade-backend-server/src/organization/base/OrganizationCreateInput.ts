/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ClientCreateNestedManyWithoutOrganizationsInput } from "./ClientCreateNestedManyWithoutOrganizationsInput";
import {
  ValidateNested,
  IsOptional,
  IsString,
  MaxLength,
  IsEnum,
} from "class-validator";
import { Type } from "class-transformer";
import { SupplierCreateNestedManyWithoutOrganizationsInput } from "./SupplierCreateNestedManyWithoutOrganizationsInput";
import { EnumOrganizationTypeField } from "./EnumOrganizationTypeField";
import { UserAccountCreateNestedManyWithoutOrganizationsInput } from "./UserAccountCreateNestedManyWithoutOrganizationsInput";

@InputType()
class OrganizationCreateInput {
  @ApiProperty({
    required: false,
    type: () => ClientCreateNestedManyWithoutOrganizationsInput,
  })
  @ValidateNested()
  @Type(() => ClientCreateNestedManyWithoutOrganizationsInput)
  @IsOptional()
  @Field(() => ClientCreateNestedManyWithoutOrganizationsInput, {
    nullable: true,
  })
  clients?: ClientCreateNestedManyWithoutOrganizationsInput;

  @ApiProperty({
    required: false,
    type: String,
  })
  @IsString()
  @MaxLength(1000)
  @IsOptional()
  @Field(() => String, {
    nullable: true,
  })
  name?: string | null;

  @ApiProperty({
    required: false,
    type: () => SupplierCreateNestedManyWithoutOrganizationsInput,
  })
  @ValidateNested()
  @Type(() => SupplierCreateNestedManyWithoutOrganizationsInput)
  @IsOptional()
  @Field(() => SupplierCreateNestedManyWithoutOrganizationsInput, {
    nullable: true,
  })
  suppliers?: SupplierCreateNestedManyWithoutOrganizationsInput;

  @ApiProperty({
    required: false,
    enum: EnumOrganizationTypeField,
  })
  @IsEnum(EnumOrganizationTypeField)
  @IsOptional()
  @Field(() => EnumOrganizationTypeField, {
    nullable: true,
  })
  typeField?: "Option1" | null;

  @ApiProperty({
    required: false,
    type: () => UserAccountCreateNestedManyWithoutOrganizationsInput,
  })
  @ValidateNested()
  @Type(() => UserAccountCreateNestedManyWithoutOrganizationsInput)
  @IsOptional()
  @Field(() => UserAccountCreateNestedManyWithoutOrganizationsInput, {
    nullable: true,
  })
  userAccounts?: UserAccountCreateNestedManyWithoutOrganizationsInput;
}

export { OrganizationCreateInput as OrganizationCreateInput };
